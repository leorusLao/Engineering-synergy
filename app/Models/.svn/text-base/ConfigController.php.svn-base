...
class ConfigController extends  Controller {
	...
	public function calendar(Request $request, Response $response)
	{
		...
		$rows = WorkCalendar::where('company_id',$company_id)->paginate(PAGEROWS);
		...
		return view('backend.calendar',array('salt' => $salt,'cookieTrail' => $cookieTrail,'rows' => $rows, 'pageTitle' => Lang::get('mowork.dashboard'),'locale' => $this->locale));
	...
	public function calendarMake(Request $request, $token, $cal_id)
	{
		...
		return view('backend.workday-setup',array('cookieTrail' => $cookieTrail,'result' => $result, 'yearList' => $yearList, 'selectedY' => $selectedY, 'selectedM' => $curMonth, 'rows' => $rows,
				'realSchedule' => $realSchedule, 'cal_name' => $cal_name, 'token' => $token, 'cal_id' => $cal_id, 'pageTitle' => Lang::get('mowork.dashboard'),'locale' => $this->locale));
	...
	public function editmeasurement(Request $request)
	{
		...
		if(!empty($id) && !empty($companyid)){
			...
			$row = CfgGaudge::info_measurement($where);
			if(empty($row)){
	...
	public function ajaxmeasurement(Request $request)
	{
		...
		if(!empty($id) && !empty($companyid)){
			...
			if(!empty($row)){
				...
				$affect = CfgGaudge::change_measurement($where,$ary);
				if($affect > 0){
			...
			$affect = CfgGaudge::insert_measurement($ary);
			if($affect > 0){
				return response()->json(array('code'=>1,'msg'=>'保存成功'));
			}else{
				return response()->json(array('code'=>2,'msg'=>'保存失败'));
	...
	public function otherSetup(Request $request)
	{
		...
		if($request->has('submit')) {
			...
			if(ConfigNumbering::isExistedPrefix($request->get('prefix'), $company_id)){
				return Redirect::back()->with('result', Lang::get('mowork.prefix_existed'));
			} else {
			 	ConfigNumbering::addNumberingSet($request->get('prefix'), $request->get('description'), $request->get('description_en'),
						$request->get('yyyy'), $request->get('mm'), $dayflag, $request->get('serial_length'), $company_id);
			}
		}
		
		$rows = ConfigNumbering::orderBy('id', 'ASC')->paginate(PAGEROWS);
		$salt = $company_id.$this->salt.$uid;
		$cookieTrail = Lang::get('mowork.system_configuration').' &raquo; '.Lang::get('mowork.other_config');
	...
	{ 
		...
		if($request->has('submit')) {
			...
			try {
				ConfigNumbering::updateNumberingSet($id, $request->get('prefix'), $request->get('description'), $request->get('description_en'), $request->get('yyyy'),
						$request->get('mm'), $dayflag, $request->get('serial_length'), $company_id);
			} catch (\Exception $e) {
