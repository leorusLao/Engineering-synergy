...
class ScanIssueCompany extends Eloquent {
	 
	protected $primaryKey = 'id';
	protected $table = 'scan_issue_company';
	...
	static public function customizeScanIssue($code, $scan_name, $date_range, $trigger, $people, $company_id )
	{
		$existed = self::isExistedCodeCompany($code, $company_id);
		 
		if($existed) {
			return self::where(array('code' => $code, 'company_id' => $company_id))
			->update(array('scan_name' => $scan_name, 'date_range' => $date_range,
					'trigger_event' => $trigger, 'people_list' => $people,'is_active' => 1));
		}
		return self::create(array('code' => $code, 'scan_name' => $scan_name, 'date_range' => $date_range, 'trigger_event' => $trigger, 'people_list' => $people, 'company_id' => $company_id
		));
	}
