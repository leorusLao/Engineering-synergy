...
@endif
  ...
  @if(count($rows))
    ...
    {{ Form::open(array('url' => '/dashboard/customer/select', 'method' => 'post', 'class' => 'form-inline', 'onsubmit' => 'return validateForm()')) }}
  ...
  {{ Form::open(array('url' => '/dashboard/customer/select', 'method' => 'post', 'class' => '', 'onsubmit' => 'return validateCheckbox()')) }}
  ...
  @if(count($rows))
     ...
     <input type="submit" name="batchSelect" class="btn btn-info" value="{{Lang::get('mowork.batch_customer')}}">
   {{ Form::close() }}
