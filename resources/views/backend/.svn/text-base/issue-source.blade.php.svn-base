...
<div class="margin-b20">
	<a href='#formholder' rel="tooltip"
		...
		data-original-title="{{Lang::get('mowork.add')}}{{Lang::get('mowork.issue_source')}}"><span
		class="glyphicon glyphicon-plus">{{Lang::get('mowork.issue_source')}}</span></a>
</div>
 ...
 @if(count($rows))
	<div class="table-responsive table-scrollable">
      <table class="table dataTable table-striped display table-bordered table-condensed">
		...
		<thead>
			<tr>
				<th>{{Lang::get('mowork.source_code')}}</th>
				<th>{{Lang::get('mowork.source_name')}}</th>
		...
		<tbody>
			...
			<tr>
				...
				<td>
				    @if($row->company_id > 0)
				    <a href="/dashboard/issue-config/issue-source/edit/{{hash('sha256',$salt.$row->id)}}/{{$row->id}}"><span class="glyphicon glyphicon-edit"></span></a> &nbsp; &nbsp;
					<a href="/dashboard/issue-config/issue-source/delete/{{hash('sha256',$salt.$row->id)}}/{{$row->id}}" onclick="return confirm('{{Lang::get('mowork.want_delete')}}')"><span class="glyphicon glyphicon-trash"></span></a>
				    @endif
				</td>			 
	...
	<div class="modal-dialog modal-sm">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title text-center">{{Lang::get('mowork.add')}}{{Lang::get('mowork.issue_source')}}</h4>
			</div>
			<div class="modal-body pull-center text-center">
				<form action='/dashboard/issue-config' method='post'
					autocomplete='off' role=form onsubmit='return validateForm();'>
					<div class="form-group">
						<input type="text" class="form-control" name="code"
							placeholder="{{Lang::get('mowork.source_code')}}"
							title="{{Lang::get('mowork.source_code')}}" id='code'>
					</div>
					<div class="form-group">
						<input type="text" class="form-control" name="name"
							placeholder="{{Lang::get('mowork.source_name')}}"
