		...
		<thead>
			<tr>
				...
				<th>{{Lang::get('mowork.node').Lang::get('mowork.name')}}</th>
  				...
  				<th>{{Lang::get('mowork.message_via')}}</th>
		...
		<tbody>
			...
			<tr>
			    ...
			    <td>
			  	...
			  	<td>
			  		<a href="#formholder" rel="tooltip" data-placement="right" data-toggle="modal" class="edit" data-placement="right" data-original-title="{{Lang::get('mowork.edit')}}" data-node_id="{{$row->node_id}}" data-node_no="{{$row->node_no}}" data-name="{{$row->name}}" data-name_en="{{ $row->name_en}}" data-category="{{$row->category}}" data-type_id="{{$row->type_id}}" data-trigger_event="{{$trigger_event[$row->trigger_event]}}" data-is_push="{{$row->is_push}}" data-key_node="{{$row->key_node}}" data-expandable="{{$row->expandables}}" >
		...
		<div class="modal-content">
			...
			<div class="modal-body pull-center text-center">
				<form action='/dashboard/project-config/node-list' method='post'
					...
					<div class="form-group input-group">
					    <div class="input-group-addon">
						<i class="livicon" data-name="doc-portrait" data-size="18" data-c="#000" data-hc="#000" data-loop="true">{{Lang::get('mowork.message_via')}}</i>
						</div>
				 		<div class="form-control">
					 		<input type="checkbox" name="sendType[]" value="1" class='sendType' />
					 		<div style="margin-right: 20%;display: inline;">
					 			{{ Lang::get('mowork.site_message') }}
					 		</div>
					 		<input type="checkbox" name="sendType[]" value="2" class='sendType' />
					 		<div style="margin-right: 20%;display: inline;">
					 			{{ Lang::get('mowork.sms') }}
					 		</div>
					 		<input type="checkbox" name="sendType[]" value="4" class='sendType' />
					 		<div style="margin-right: 20%;display: inline;">
					...
					<input type="hidden" name="trigger_event" value="0">
					<input name="_token" type="hidden" value="{{ csrf_token() }}">
				</form>
				<!-- 
				<div id="tmpfile">
...
$("[rel=tooltip]").tooltip({animation:false});
    ...
    $(function(){
    	...
    	$('.edit').click(function(){
    		...
    		var tmp = obj.data('trigger_event').toString();
    		$('.sendType').each(function(){
    	...
    	$('.add').click(function(){
    		$('.add_edit').find('input[name="node_id"]').remove();
    		$('.add_edit').find('input[type="text"]').val('');
    	...
    	$('.click').click(function(){
    		...
    		$('.sendType').each(function(){
    			if($(this).is(':checked')) {
    				trigger_event += parseInt($(this).val());
    			}
    		});
    		$('input[name="trigger_event"]').val(trigger_event);
