...
<div class="col-xs-12">
	...
	@if(count($rows))
	    {{ Form::open(array('url' => '/dashboard/progress-feed-in', 'method' => 'post', 'class' => 'form-inline')) }}
        ...
        {{ Form::close()}}
            ...
            <th nowrap="nowrap">{{Lang::get('mowork.progress_input')}}</th>
            </tr>
			</thead>
			...
			@foreach($rows as $row)
				...
				<td >
					@if($row->plan_status == 4) {{Lang::get('mowork.agree') }} @endif
			    </td>
				<td >{{ substr($row->start_date,0,10) }}</td>
				<td nowrap="nowrap">{{ substr($row->end_date,0,10) }}</td>
				 
				<td class="text-center"> 
	            <a href='#formholder' data-toggle="modal" data-book-id={{$row->task_id}}><span class="glyphicon glyphicon-edit"></span></a>
			 	</td>
 	 		</tr>
...
<script type="text/javascript">
    ...
    function isValidDate(dateString)
    {
      ...
      if(!dateString.match(regex_date))
      {
          return false;
      }
