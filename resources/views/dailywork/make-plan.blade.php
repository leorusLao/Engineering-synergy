...
@endif
 ...
 @endif
 <div class="margin-b10"><span>{{Lang::get('mowork.apply')}}
	...
	</a>{{Lang::get('mowork.plan_easy')}}.</span>
	<span class="text-danger">{{Lang::get('mowork.cal_warning')}}</span>
 </div>
  ...
  <div class="margin-t20 text-center">
    <button style="cursor: pointer" id="save">
        {{ Lang::get('mowork.save') }}
    </button>&nbsp;&nbsp;&nbsp;&nbsp;
    <span onclick="terminate()" style="cursor: pointer">
      {{Lang::get('mowork.close')}}
    </span>
  </div>
</div>
...
<div class="modal fade" id="taskedit" style="z-index: 9999">
    ...
    <tr><td class="form-inline"><span>{{Lang::get('mowork.department')}}</span>
            <span>
            <select class="form-control" name="department" id="js11">
                 
    		    </select>
            </span>     
        </td>
        <td>{{Lang::get('mowork.leader')}}
           <select class="margin-t20" id='pickman' name="people[]">
...
<div class="space">
    ...
    <input type="button"  class="btn btn-info btn_update" value="{{Lang::get('mowork.confirm')}}" />
...
<form action="/dashboard/plan-task-update" method="post">
    {{ csrf_field() }}
    <input type="hidden" name="update_data" value="" />
    <input type="submit" class="save" value="{{ Lang::get('mowork.confirm') }}" style="display:none;" />
</form>
...
<script type="text/javascript">
    ...
    $(function(){
	         			  ...
	         			  $.ajax({
	         	  	          type:"POST",
	         	  	           ...
	         	  	           success : function(res) {
  	         	  	        	...
  	         	  	        	var node_name = res.node_name;
                            var departments = res.department;
                            var options = '';
                            var departmentOptions = '';
                            if(Object.keys(node_name).length) {
                              for(var iii in node_name){
                                if(iii == selected_node) {
                                  options += "<option value='" + iii + "' selected>" + node_name[iii] + "</option>";
                                }else {
                                  options += "<option value='" + iii + "'>" + node_name[iii] + "</option>";
                                }
                              }
                            }
                            if(Object.keys(departments).length) {
                              for(var iii in departments)
                              {
                                if(result[11] == iii) {
                                  departmentOptions += '<option value="' + iii + '" selected>' + departments[iii] + '</option>';
                                }else {
                                  departmentOptions += '<option value="' + iii + '">' + departments[iii] + '</option>';
                                }
                              }
                            }
                           
                            $('#js8').html(options);
                            $('#js11').html(departmentOptions);
        ...
        $('#js9').change(function() {
              ...
              if(Object.keys(departments).length) {
                for(var ii in departments)
                {
                  departmentOptions += '<option value="' + ii + '">' + departments[ii] + '</option>';
                }
              }
      ...
      $('.btn_update').click(function(){
        ...
        arrStr += '"task_id":"' + $('#task_id').val() + '",';
        arrStr += '"node_id":"' + $('#js8').val() + '",';
        arrStr += '"plan_id":"' + $('input[name="plan_id"]').val() + '",';
        arrStr += '"nodetype":"' + $('#js9').val() + '",';
        arrStr += '"department":"' + $('#js11').val() + '",';
        arrStr += '"people":"' + $('#pickman').val() + '",';
        arrStr += '"outsource":"' + ($('#outsource').is(':checked') ? $('#outsource').val() : 0) + '",';
        arrStr += '"supplier":"' + $('#supplier').val() + '",';
        arrStr += '"start_date":"' + $('#js13').val() + '",';
        arrStr += '"end_date":"' + $('#js14').val() + '",';
        arrStr += '"milestone":"' + ($('#milestone').is(':checked') ? $('#milestone').val() : 0) + '",';
        arrStr += '"keynode":"' + ($('#keynode').is(':checked') ? $('#keynode').val() : 0) + '",';
        arrStr += '"condition":"' + $('#condition').val() + '"},';
      ...
      $('#save').click(function(){
          if(update_data == '['){ alert("{{Lang::get('mowork.has_no').Lang::get('mowork.update')}}");return;}
          update_data = update_data.substr(0, update_data.length - 1) + ']';
          $('input[name="update_data"]').val(update_data);
