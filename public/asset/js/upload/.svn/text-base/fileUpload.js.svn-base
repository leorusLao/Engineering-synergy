...
var uploadTools = {
    ...
    "getInitOption":function(uploadId){
        ...
        var initOption={
            "uploadId":uploadId,
            "uploadUrl":"#",//必须，上传地址
            "deleteUrl":"http://localhost/dashboard/delete-files",//必须，上传地址
    ...
    "initWithUpload":function(opt){
        ...
        if(!opt.isHiddenUploadBt){
            ...
            $("#"+uploadId+" .uploadBts .uploadFileBt i").css("color","#A9B6BC");
        }
    ...
    "initWithCleanFile":function(opt){
        ...
        if(!opt.isHiddenCleanBt){
            ...
            $("#"+uploadId+" .uploadBts .cleanFileBt i").css("color","#A9B6BC");
        }
    },
    /**
     * 初始化选择文件按钮
     ...
     * @param fileType 文件类型
     * @param fileName 文件名字
     * @param isImgUrl 如果是文件时的文件地址默认为null
     * @param gou 如果是1则是已上传的文件  0则是未上传的文件
     */
    "getShowFileType":function(isImg,fileType,fileName,isImgUrl,fileCodeId,gou){
        ...
        if(isImg){
            if(isImgUrl!=null&&isImgUrl!="null"&&isImgUrl!=""){//图片显示类型
                showTypeStr = "<a href='" + isImgUrl + "' target='_blank'><img src='"+isImgUrl+"'/></a>";
            }
        }else if(fileType == 'doc' || fileType == 'docx'){ 
                showTypeStr = "<a href='" + isImgUrl + "'><img src='http://localhost/uploads/common/word.jpg'/></a>";
        }
        var modelStr="";
        if(gou==1){
            modelStr+="<div class='fileItem'  fileCodeId='"+fileName+"' systemFile=1>";
        }else if(gou==0){ 
            modelStr+="<div class='fileItem'  fileCodeId='"+fileName+"' systemFile=0>";
        }
        ...
        modelStr+="</div>";
        modelStr+="<div class='status span_shanch'>";
        modelStr+="<i class='iconfont icon-shanchu'></i>";
        if(gou==1){
            modelStr+="<i class='iconfont icon-gou'></i>";
        }else if(gou==0){ 
            modelStr+="<i class='iconfont jiade'></i>";
        }
        modelStr+="</div>";
    ...
    "initWithDeleteFile":function(opt){
        var uploadId = opt.uploadId;
        $("#"+uploadId+" .fileItem .status i.icon-shanchu").on("click",function(){
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeStr,fileTypeArray)){
                ...
                if(imgtest.test(fileList[i].type)){
                    ...
                    var fileModel = uploadTools.getShowFileType(true,fileTypeUpcaseStr,fileList[i].name,imgUrlStr,fileListArray.length,0);
                    $(boxJsObj).append(fileModel);
                }else{
                    var fileModel = uploadTools.getShowFileType(true,fileTypeUpcaseStr,fileList[i].name,null,fileListArray.length,0);
                    $(boxJsObj).append(fileModel);
                }
                uploadTools.initWithDeleteFile(opt);
    ...
    "showUploadProgress":function(opt,bytesRead,percent){
        ...
        for(var i=0;i<fileListArray.length;i++){
            ...
            if(testbytesRead<0){
                ...
                $(".box .fileItem .status>i:odd").addClass("iconfont icon-gou");
                $(".box .fileItem .status>i:even").addClass("iconfont icon-shanchu");
                $(".box .fileItem").attr("systemFile",1);
                if(opt.showFileItemProgress) {
                    $("#" + uploadId + " .box .fileItem[fileCodeId='" + i + "'] .progress>div").css("width", "100%");
                }
                bytesRead = bytesRead-fileListArray[i].size;
    ...
    "uploadFile":function(opt){
        ...
        ary_new = [];
        for(var i=0;i<fileList.length;i++){
            if(fileList[i]!=null && fileList[i].name.length != 0){ 
                ary_new.push(fileList[i]);
            }
        }
        for(var i=0;i<ary_new.length;i++){
            num_length = 0;
            formData.append("file_"+i,ary_new[i]);
        }
        length = ary_new.length;
...
var uploadEvent = {
    ...
    "deleteFileEvent":function(opt,obj){
        ...
        var systemFile = fileItem.attr("systemFile");
        var fileListArray = uploadFileList.getFileList(opt);
        formData = new FormData();
        formData.append("filename",fileCodeId);
        if(systemFile==1){ 
            $.ajax({
                type:"post",
                url:deleteUrl,
