...
if (isset($_GET["code"])) {  
  ...
  if($access_token = $oauth->getAccessToken('code', $code)){
	...
	$forward_url = "http://test.mowork.cn/login/wechat";	
...
EOD;
	...
	echo '<script>window.location.href = "http://test.mowork.cn/login"</script>';
...
$ch = curl_init('http://test.mowork.cn/api/wechat/login');                                                                     
